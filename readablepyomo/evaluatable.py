def _create_code(item, model_variable_name = "model", index_map = dict()):
	if "create_code" in dir(item):
		#print(item.create_code(model_variable_name, index_map))
...
class UpperBoundComparison(Evaluatable):
	...
	def create_code(self, model_variable_name = "model", index_map = dict()):
		return _create_code(self.expression, model_variable_name, index_map) + " <= " + _create_code(self.upper_bound, model_variable_name, index_map)
...
class LowerBoundComparison(Evaluatable):
	...
	def create_code(self, model_variable_name = "model", index_map = dict()):
		return _create_code(self.expression, model_variable_name, index_map) + " >= " + _create_code(self.lower_bound, model_variable_name, index_map)
			
	def get_unbound_sets(self):
		return self.expression.get_unbound_sets() + _get_unbound_sets(self.lower_bound)
		
class EqualityComparison(Evaluatable):
	def __init__(self, expression, value):
		self.expression = expression
		self.value = value
		
	def create_code(self, model_variable_name = "model", index_map = dict()):
		return _create_code(self.expression, model_variable_name, index_map) + " == " + _create_code(self.value, model_variable_name, index_map)
			
	def get_unbound_sets(self):
		return self.expression.get_unbound_sets() + _get_unbound_sets(self.value)
